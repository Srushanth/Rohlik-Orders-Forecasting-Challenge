...
with mlflow.start_run(experiment_id=experiment_id, log_system_metrics=True) as run:
    ...
    regressor = TabularPredictor(label="orders").fit(
        x_train,
        hyperparameters=hyperparameters,
        time_limit=3600,  # Time limit for the fit process in seconds
        presets='best_quality'
        # Other options: 'high_quality_fast_inference_only_refit', 'good_quality_faster_inference_only_refit',
    ...
    mlflow.log_param(key="model", value="TabularPredictor-HPT")
